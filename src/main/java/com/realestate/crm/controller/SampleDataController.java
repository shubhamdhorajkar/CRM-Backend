...
public class SampleDataController {
    ...
    // READ ONE
    @GetMapping("/{id}")
    public ResponseEntity<SampleData> getById(@PathVariable Long id) {
    ...
    // UPDATE
    @PutMapping("/{id}")
    public ResponseEntity<SampleData> update(@PathVariable Long id, @RequestBody SampleData updated) {
        return repository.findById(id)
                .map(existing -> {
                    updated.setId(id);
    ...
    public ResponseEntity<String> uploadExcel(@RequestParam("file") MultipartFile file) throws IOException {
        List<SampleData> siteVisits = new ArrayList<>();
        ...
        try (Workbook workbook = new XSSFWorkbook(file.getInputStream())) {
            ...
            for (int i = 1; i <= sheet.getLastRowNum(); i++) { // skip header
                ...
                SampleData visit = new SampleData();
